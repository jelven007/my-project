...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/dealers` | 按车型/城市查询可承接下定的启用门店;仅返回 `available` 布尔值,不返回数量 |
...
18. 用户可创建金额为 0 的下定订单,且不进入任何支付流程。
19. 并发下定不会造成超卖,重复提交不会生成重复订单。
20. 用户只能查看和取消自己的订单;取消或超时会正确释放库存。
21. 无库存、经销商停用或车型下线时不能下定。
22. 用户注册验证码通过火山引擎短信服务真实到达目标手机号。
23. 非测试环境配置 mock Provider 时服务拒绝启动。
24. API、日志和数据库均不出现明文验证码或完整手机号。
25. 发送频率、验证码有效期、单次使用和 5 次验证失败失效规则生效。
26. 火山引擎短信服务拒绝、超时和回执失败均有可追踪状态及监控告警。
