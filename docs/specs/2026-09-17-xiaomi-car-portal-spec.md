...
- 在线支付、付费定金、退款和正式购车合同。
- 完整车辆选配器。
- 门户库存查询、库存数量展示与库存维护;库存仅由后台运营系统管理。
- 与经销商 ERP/DMS 的库存自动同步;本期库存仍由后台人工录入。
- 微信以外的第三方登录。
- 中文以外的语言包和语言切换。
...
### 3.8 dealers
`id`, `name`, `code`, `province`, `city`, `address`, `phone`, `longitude`, `latitude`, `business_hours`, `status`, `created_at`, `updated_at`。门户仅展示 `status=active` 且可承接所选车型意向订单的门店。
...
该模型属于后台与服务端内部数据。门户 API 不返回 `total_qty`、`reserved_qty`
或精确可用数量,仅返回门店对指定车型是否可下定以及提交订单所需的不透明
`inventoryId`。
...
> 来源:小米汽车官网 `xiaomiev.com`,核对时间 2026-09。图片从对应车型官方页面下载后缓存为门户本地静态资源,避免运行时依赖外部 CDN。

| 车型 | 起售价 | 核心参数 | 官网主图 |
|------|--------|----------|----------|
| 小米 SU7 | ¥219,900 | 720km CLTC,320PS | `/images/cars/su7.jpg` |
| 小米 YU7 | ¥235,500 | 835km CLTC,5.88s | `/images/cars/yu7.jpg` |
| 小米 SU7 Ultra | ¥529,900 | 630km CLTC,1548PS,2.1s,350km/h | `/images/cars/su7-ultra.jpg` |
...
### 5.2 全局导航
- Logo、首页、SU7、YU7、SU7 Ultra、预约试驾。
...
- 车型概览不提供固定指向某一车型的统一“车型详情”链接;用户通过对应车型卡片进入详情。
...
- 提供“立即下定”与“预约试驾”CTA。
- “立即下定”进入交付门店选择流程;车型详情不展示库存数量或占用状态。
- 未发布或不存在车型返回友好 404。

### 5.5 交付门店选择 `/order`
- 从车型详情进入,车型由 `carId` 与 `slug` 唯一确定。
- 支持按城市筛选可承接该车型意向订单的启用门店。
- 展示门店名称、地址、营业时间和联系电话。
- 不展示库存总量、已占用量、精确可用量或库存更新时间。
- 无可选门店时引导用户预约试驾或稍后重试。

### 5.6 0 元下定 `/orders/confirm`
- 用户先选择车型和交付门店,确认下定时若未登录则跳转登录并保留回跳地址。
...
- 后端库存校验失败时返回明确提示,但门户不回显库存数量;不得生成无库存订单。
...
### 5.8 预约试驾 `/test-drive`
- 受登录保护。
- 字段:车型、门店、联系人、联系电话、期望日期和备注。
- 车型仅来自后台已发布车型;选择车型后,门店从后台启用经销商及其有效车型关联中动态加载。
- 门店必须与所选车型匹配;车型下线、经销商停用或车型门店关联停用后不再提供新预约。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/dealers` | 按车型/城市查询可承接下定或试驾的启用门店;仅返回 `available` 布尔值,不返回数量 |
...
10. 后台发布或下线车型后,门户重新加载或重新聚焦时立即重新校验,并同步更新首页、详情、试驾和下定入口。
...
16. 用户可从车型详情进入下定页并按城市选择可承接的交付门店。
17. 门户页面和公开 API 均不展示库存总量、已占用量或精确可用数量。
18. 预约试驾的门店来自后台启用经销商,并且必须与用户选择的已发布车型匹配。
19. 用户可创建金额为 0 的下定订单,且不进入任何支付流程。
20. 并发下定不会造成超卖,重复提交不会生成重复订单。
21. 用户只能查看和取消自己的订单;取消或超时会正确释放库存。
22. 无库存、经销商停用或车型下线时不能下定。
23. 用户注册验证码通过火山引擎短信服务真实到达目标手机号。
24. 非测试环境配置 mock Provider 时服务拒绝启动。
25. API、日志和数据库均不出现明文验证码或完整手机号。
26. 发送频率、验证码有效期、单次使用和 5 次验证失败失效规则生效。
27. 火山引擎短信服务拒绝、超时和回执失败均有可追踪状态及监控告警。

### 9.1 测试
- 单元测试:密码策略、验证码哈希/过期/次数、短信限流、Token、微信 state、库存计算和订单过期释放。
- API 集成测试:火山引擎 SMS Provider 契约、状态报告回调、注册、微信绑定、登录、库存行锁、订单幂等和预约隔离。
- E2E:浏览车型、选择交付门店、微信/手机号登录、0 元下定、取消订单、提交预约和会话恢复。
